...
void setup() {
  ...
  // Load the FlightHub logo; ensure it is in the "data" folder.
  ...
  // Create the AirportSelectorMenu with the logo.
...
void keyPressed() {
  if (screenMode == SCREEN_SELECTION && airportSelector.searchFocused) {
    ...
    if (key == BACKSPACE) {
      ...
      // Optionally add handling for ENTER here.
    }
    else {
      char c = key;
      insertKeyChar(c);
