...
class GraphSelectorMenu {
  ...
  void display() {
    ...
    String fullOrigin = graphScreen.airportNameLookup().get(airport);
  ...
  void mousePressedMenu(float mx, float my) {
    if (mx >= 10 && mx <= 90 && my >= 10 && my <= 40) {
      if (inMenu) {
        graphScreen.screenMode = graphScreen.SCREEN_SELECTION;
      } else {
        data.filterDate = null;
...
void drawBarChart(float x, float y, float w, float h) {
  ...
  Collections.sort(destList, (a, b) -> {
    String nameA = graphScreen.airportNameLookup().get(a.getKey());
    if (nameA == null) nameA = a.getKey();

    String nameB = graphScreen.airportNameLookup().get(b.getKey());
    if (nameB == null) nameB = b.getKey();
  ...
  for (int i = 0; i < itemsToShow; i++) {
    ...
    String fullLabel = graphScreen.airportNameLookup().get(code);
