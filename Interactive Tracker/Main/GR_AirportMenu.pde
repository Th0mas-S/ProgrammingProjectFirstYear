// AirportSelectorMenu.pde
// Handles displaying, searching, sorting, and selecting airports.
...
class AirportSelectorMenu extends Screen {
    ...
    // Layout values – recalculated dynamically.
    ...
    AirportSelectorMenu(String[] airports, StringLookup lookup) {
      println("AirportSelectorMenu created");
      ...
      display();
      //drawAirportList();  
      handleMouseDraggedInSearch(float mx);
      handleSliderMousePressed(float mx, float my);
      handleSliderMouseDragged(float mx, float my);
      
      handleSliderMouseReleased();
      
      updateSlider(float my);
  }
    
    // Recalculate layout positions based on current width and height.
    void recalcLayout() {
      listWidth = 0.5 * width;  
      itemHeight = 80;
    ...
    void display() {
      background(255);  //testing purposes 
      println("AirportSelectorMenu is being displayed");
      println("Number of airports: " + airports.length);
   ...
   void drawAirportList() {
    ...
    for (int i = 0; i < itemsToShow; i++) {
      ...
      if (index < filteredAirports.length) {
        ...
        // Dynamically reduce font size to prevent overflow
        float fontSize = 24;
        textSize(fontSize);
