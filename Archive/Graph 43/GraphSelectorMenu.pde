...
// --------------------------------------------------------------------------
// The GraphSelectorMenu class (your menu and UI)
// --------------------------------------------------------------------------
...
class GraphSelectorMenu {
  ...
  void display() {
    ...
    if (inMenu) {
      ...
      float headingBaseY = startY - 80; // Move the headings well above the widgets
      ...
      headingBaseY += 35; // Add a bit of spacing after the main heading
      ...
      if (data.totalFlights == 0) {
        ...
        switch (selectedGraph) {
          ...
          case 4: new RadarChartScreen(airport, data, animationProgress, true).display(gx, gy, gw, gh); break;
    ...
    if (inMenu) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (mx > bx && mx < bx + btnWidth && my > by && my < by + btnHeight) {
          selectedGraph = i;
          inMenu = false;
          graphStartTime = millis();  // Start animation timer.
          animationProgress = 0;
          data.filterDate = null;
          lastSelectedDate = null;
          data.process(airport);
