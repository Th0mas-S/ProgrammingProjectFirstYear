// LineGraphScreen.pde
// A class for drawing the line graph (Hourly Flight Counts) screen with:
// 1) Two-tone fill under the line,
// 2) A y-axis label,
// 3) Extra spacing for the x-axis label,
// 4) Shifts the y-axis label further to the left so it doesn't overlap numeric ticks,
// 5) Moves only the first data point’s label to the right so it doesn’t touch the y-axis.
