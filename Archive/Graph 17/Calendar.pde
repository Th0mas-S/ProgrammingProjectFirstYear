class CalendarDisplay {
  ...
  void display() {
    ...
    float dayNamesY = headerY + headerHeight + 10;
    for (int i = 0; i < 7; i++) {
  ...
  // Called by mousePressed() in Main or GraphScreen.
  // Returns true if the calendar handled the click.
boolean mousePressed() {
  if (mouseX >= x && mouseX <= x + w && mouseY >= y && mouseY <= y + h) {
    float headerY = y + 20;
    float headerHeight = 40;
    float headerCenterY = headerY + headerHeight / 2;
    // Left arrow: change month but do not count as day selection.
    if (month > 0 &&
    ...
    // Right arrow: change month but do not count as day selection.
    if (month < 11 &&
    ...
    float cellW = (w - 20) / 7.0;
    float cellH = 25;
    float gridStartY = headerY + headerHeight + 10 + 15;
    ...
    for (int i = 1; i <= daysInMonth; i++) {
      ...
      if (mouseX >= cellX && mouseX < cellX + cellW &&
        ...
        return true;  // Return true only when a day is clicked.
      }
    }
  }
  return false;
  ...
  // Update previousMonth to reset selectedDay to 1
  void previousMonth() {
    if (month > 0) {
      month--;
      selectedDay = 1;
  ...
  // Update nextMonth to reset selectedDay to 1
  void nextMonth() {
    if (month < 11) {
      month++;
      selectedDay = 1;
