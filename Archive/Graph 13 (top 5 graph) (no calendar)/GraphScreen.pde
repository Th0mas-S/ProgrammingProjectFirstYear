...
// Helper: Adjust text size so that all lines fit within maxWidth.
...
class GraphScreen {
  ...
  // When inMenu is true, display the graph selection menu.
  boolean inMenu = true;
  int selectedGraph = 0;  // 0: Pie, 1: Line, 2: Bar (Top 5 Destinations), 3: Grouped
  ...
  void display() {
    ...
    if (inMenu) {
      ...
      switch(selectedGraph) {
        ...
        case 2: title = "Top 5 Destinations for " + fullOrigin; break;
  ...
  // Called from mousePressed() in Main.pde.
  ...
  void drawLineGraph(float x, float y, float w, float h) {
    ...
    for (int i = 0; i < visiblePoints - 1; i++) {
      ...
      float x2 = xpos[i + 1];
      float y2 = ypos[i + 1];
      fill((i % 2 == 0) ? 160 : 100, (i % 2 == 0) ? 190 : 150, 255);
      beginShape();
      vertex(x1, y1);
      vertex(x2, y2);
  ...
  // --------------- Graph 2: Bar Chart – Top 5 Destinations ---------------
...
// Graph 2: Bar Chart – Top 5 Destination Airports with three-line labels
void drawBarChart(float x, float y, float w, float h) {
  float marginLeft = 60, marginRight = 30, marginTop = 40, marginBottom = 150;

  // Count flights to each destination for the selected airport.
  ...
  // Draw bars for each top destination.
  for (int i = 0; i < itemsToShow; i++) {
    ...
    // Display the flight count above each bar.
    fill(0);
    textSize(18);
    textAlign(CENTER, BOTTOM);
    text(count, bx + barWidth / 2, by - 6);
    
    // Prepare three-line label:
    //  - First line: Name before any '(' if available.
    //  - Second line: The text inside parentheses.
    //  - Third line: The destination code.
    ...
    // Use the helper to determine the fitted text size.
  ...
  // --------------- Graph 3: Grouped Bar Chart – Airline Performance ---------------
  void drawGroupedBarChart(float x, float y, float w, float h) {
    ...
    // Optional: Draw month labels (you can remove this if not needed).
    textAlign(CENTER, TOP);
    textSize(16);
