...
void draw() {
  ...
  // Draw the cycling loading text above the bar.
  hint(DISABLE_DEPTH_TEST);
  fill(12, 190, 12);
  textAlign(CENTER, CENTER);
  textSize(40);
  int loadingIndex = int((millis() / 500) % 4);
  String[] loadingStates = { "",".", "..", "..." };
  String loadingText = loadingStates[loadingIndex];
  
  
  
  // Position the text above the loading bar (adjust coordinates as needed)
  text(loadingText, 1023, 930);
  
  
  
  fill(12, 190, 12);
  textSize(30);
  text("Loading", 950, 935);  
  hint(ENABLE_DEPTH_TEST);
